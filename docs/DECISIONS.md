...
**D-063 · Phone sweep of every page (requested 22 Sept 2026).** After D-060 fixed the oversized cards, every one of the 47 pages was reviewed at 390px from screenshots, and the fixes were grouped by file so each shared component changed once. Same rule as D-060: phone only, below 640px, with an `sm:` override restoring today's value; nothing removed or hidden; tap targets 44px, body text 16px.
- **Proved phone-only.** Every element's box was measured at 640, 768, 1024 and 1440px on the build before the sweep and on the build after it: not one box moved or resized on any page. (A pixel comparison was tried first and dropped; the web font sometimes loaded late in a capture, so pixels changed between two runs of the same build. Box geometry, measured after `document.fonts.ready`, repeated exactly.)
- **What changed on phones:**
  - Page H1 and section h2 take one step down below 640px (34px and 28px up to 400px wide, growing to today's size at 640px, so there's no jump). At 32-40px, Archivo at 112% split balanced headings into four or five lines with a lone word.
  - The header-to-content gap under section headings is 32px (was 48); the closing CtaBand and the Apply page's bottom padding follow the 56px section rhythm instead of 96px.
  - The homepage carousels (pillars, featured industries) now rest on the 20px gutter instead of flush against the screen edge.
  - Horizontal process steps become the homepage's vertical numbered list on phones; the ISOs & agents programme steps no longer stretch to 200px each.
  - The industries hub's filter chips are one swipeable row (they wrapped into five rows); small industry cards size to their content.
  - Denser rows in the capability tabs, the flow diagram, the chip bands, the comparison tables' phone cards, the About values and audiences, the careers roles and culture list, the client stories, the FAQ, and the contact and apply forms.
- **Result at 390px:** every page is shorter except Privacy (+12px, a heading gap); 5.3% less scrolling across all 47 pages, 16.5% on the industries hub, 7-10% on the partner pages.
- **Left alone:** the footer's top padding (low priority, and D-055 is recently approved), and the "Why Operate with Beyond." split layout.
